...
public class MapTest {
    ...
    /**
     * Программа вычисляет ячейку для объектов по хешкоду (хешкод & количество ячеек в массиве)
     * далее проверяет есть ли в ячейке значение
     * значение null - записывает значение в ячейку
     * если значение есть, то программа сравнивает хэшкоды ячеек, если хэшкоды равны(такое бывает так как количество кодов ограниченно) то сравнивает
     * значение по equals если equals дает true, значит записывается новое значение в ячейку, если equals false значит в эту ячейку добавляется еще одно значение
     * добавление может происходить как связным списком, так и в следующий индекс массива, зависит от коллекции
     * именно поэтому добавляют параметр заполненности массива mapы, чтобы увеличивать ее для большей эффективности получения элементов
     */
    @Test
